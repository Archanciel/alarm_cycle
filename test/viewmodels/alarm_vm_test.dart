...
void main() {
  group('checkAlarmsPeriodically', () {
    test(
      ...
      // The last alarm time real should be now. In order to avoid
      // flaky tests, we don't check equality, but that the < 1 second
      // difference
      final Duration difference =
          alarm.lastAlarmTimeReal!.difference(now).abs();
      expect(difference.inSeconds <= 1, true);

      // checking that the next alarm time was correctly calculated
    ...
    test(
      ...
      () async {
        ...
        Alarm alarm = Alarm(
          ...
          lastAlarmTimePurpose: lastAlarmTimePurpose,
          lastAlarmTimeReal:
              lastAlarmTimePurpose.add(const Duration(minutes: 7)),
          nextAlarmTime: lastAlarmTimePurpose.add(periodicDuration),
          periodicDuration: periodicDuration,
          audioFilePathName: '',
        );
